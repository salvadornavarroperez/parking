...
switch ($_SERVER['REQUEST_METHOD']) {
	...
	case 'GET':
		...
		$metodos_pago = $metodo_pago->get($params);
	...
	case 'POST':
		...
		$insert_id = $metodo_pago->insert($params);
	...
	case 'PUT':
		...
		if(!isset($params) || !isset($_GET['id_metodo']) || empty($_GET['id_metodo'])){
			$response = array(
				'result' => 'error',
				'details' => 'Error en la solicitud'
			);
		...
		$metodo_pago->update($_GET['id_metodo'], $params);
	 ...
	 */
	case 'DELETE':
		if(!isset($_GET['id_metodo']) || empty($_GET['id_metodo'])){
			$response = array(
				'result' => 'error',
				'details' => 'Error en la solicitud'
			);
		...
		$metodo_pago->delete($_GET['id_metodo']);
